...
def one_min_one(y):
    one_min_one_y = np.zeros((y.size, int(y.max()) + 1)) - 1
    one_min_one_y[np.arange(y.size), y.astype(int)] = 1 # each row of the minus one vector which corresponds to classified number will become 1
...
W1, b1, W2, b2 = neural_network(train, train_l, 100, 0.09, 100, 0, "tanh")
